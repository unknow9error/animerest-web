...
export const AnimeList: FC<AnimeListProps> = ({ data, title }) => {
    const navigate = useNavigate();
    const [addAction] = useActionMutation();

    const handleRedirect = (animeId: string) => {
        return () => {
    ...
    return (
        <Row gutter={[0, 0]} className={styles.animeListWrapper}>
            <Col span={24}>
                <p>{title}:</p>
                <Carousel
                    arrows
                    prevArrow={<ArrowLeftOutlined color="white" />}
                    nextArrow={<ArrowRightOutlined color="white" />}
                    autoplay
                    slidesToShow={7}
                    dots={false}
                    slidesPerRow={1}
                    ...
                    {data?.map((anime) => {
                        return (
                            ...
                            >
                                ...
                                >
                                    <div
                                        onClick={handleRedirect(anime.code)}
